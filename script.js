...
function rnd2(min, max) { return min + Math.floor(Math.random() * (max - min)); }
function percent(value) { if (rnd(100.0) < value) return true; else return false; }
function gen_fileName() {
    let fileName = '';
    for (let i = 0; i < 20; i += 1) {
        if (percent(50)) fileName += String.fromCharCode(rnd2(48, 57));
        else
            if (percent(50)) fileName += String.fromCharCode(rnd2(65, 90));
            else
                fileName += String.fromCharCode(rnd2(97, 122));
    }
    return fileName;
}
...
let lastX = 0, lastY = 0, hue = 0, slideID = 0, undoID = 0;
let isDrawing = false, isRecording = false;
...
function login() {// Авторизация и загрузка лекций пользователя

};

function load_profile() {// Загрузка профиля пользователя

};

function load_lection_list() {// Загрузка списка лекций пользователя

};

function gen_slideList() {// Генерация тестового списка слайдов
    for (let i = 0; i < 100; i++) {
        slideList.push('Слайд ' + i + ', videoURL, pictureURL, no');
    }
};
function load_slideList() {// Загрузка списка слайдов лекции пользователя
    update_slideList();
...
function onImageChange(e) {// Изменение рисунка
    e.preventDefault();
...
function start_drawing(e) {// Начало рисования
    if (!isRecording) return;
    isDrawing = true;
    [lastX, lastY] = [e.offsetX, e.offsetY];
};

function drawing(e) {// Процесс рисования
...
function stop_drawing() {// Прекращение рисования
    if (!isRecording) return;
...
function redo() {// Повтор действия
    if (undoID > undoList.length - 2) return;
    if (undoID == undoList.length - 2) redoImg.src = '/resources/images/redoOff.png';
    undoImg.src = '/resources/images/undo.png';
    undoID++;
    context.putImageData(undoList[undoID][0], 0, 0);
    updateDebug();
...
function start_recording() {// Начало записи
...
function onStopMediaRecorder() {// Остановка и сохранение записи, обновление страницы
...
function play_lection() {// Воспроизведение видео и пометок
...
function get_slide(num, num2) {// Получить данные слайда
    return slideList[num].split(', ')[num2];
};

function set_slide(num, num2, value) {// Задать данные слайда
...
function update_slideList() {// Обновить поле со слайдами
    pageSlides.innerHTML = '<ol>';
    for (i = 0; i < slideList.length; i++) {
        let done = '';
        if (get_slide(i, 3) == 'yes')
            done = ' ✓';
        if (i == slideID)
            pageSlides.innerHTML += '<li class=line onclick=change_slide_name(' + i + ')><strong>' + get_slide(i, 0) + done + '</strong></li>';
        else
            pageSlides.innerHTML += '<li class=line onclick=update_slide(' + i + ')>' + get_slide(i, 0) + done + '</li>';
    }
    pageSlides.innerHTML += '</ol>';
};

function initVideo() {// Инициализация видео
...
function update_slide(num) {// Переключение на заданный слайд
...
function del_slide() {// Удалить слайд
    if (slideList.length > 1) {
        slideList.splice(slideID, 1);
        if (slideID == slideList.length)
            slideID--;
        update_slide(slideID);
    }
    else
        alert('Остался всего один слайд!');
};

function new_slide() {// Создать слайд
    slideID++;
    slideList.splice(slideID, slideList.length - slideID,
        input('Введите название нового слайда') + ', videoURL, pictureURL, no');
    update_slide(slideID);
};

function change_slide_name(num) {// Изменить название слайда
...
document.addEventListener('keydown', (e) => {// Управление с клавиатуры
    //alert(e.code);
    if (e.code === 'Delete') {
        del_slide();
